...
// Show all users loged in the last 24 hours
cli.responders.listUsers = function(){
    
    // Lookup the users folder
    _data.list('users',function(err,usersEmails){
        if(!err && usersEmails && usersEmails.length > 0){
            // We delete the .gitkeep in orders folder from the list of orders ids
            var gitkeepIndex = usersEmails.indexOf('.gitkeep');
            usersEmails.splice(gitkeepIndex,1);
            // Command response title 
            console.log("Users Signed Up - in the last 24 hours by Email\n");
            usersEmails.forEach(function(user){
                _data.read('users',user,function(err,userData){
                    if(!err && userData){
                        // If the order was placed less than 24 hours ago proceed
                        if ((userData.date + 1000 * 60 * 60 * 24) >= Date.now()){
